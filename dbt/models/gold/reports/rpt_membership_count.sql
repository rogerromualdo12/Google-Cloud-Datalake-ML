WITH subs AS (
    SELECT
        ...
        concat(substr(os.export_date, 1, 4), "-", substr(os.export_date, 5, 2)) AS period,
        CASE
        ...
        CASE
            WHEN
                sni_stripe.isBasePrice_(s.items_data_0_price_id) IS NOT null
                THEN s.items_data_0_plan_product
            WHEN
                sni_stripe.isBasePrice_(s.items_data_1_price_id) IS NOT null
                THEN s.items_data_1_plan_product
            WHEN
                sni_stripe.isBasePrice_(s.items_data_2_price_id) IS NOT null
                THEN s.items_data_2_plan_product
        END AS base_plan,
        concat(
            substr(os.export_date, 1, 4),
            "-",
            substr(os.export_date, 5, 2),
            "-",
            substr(os.export_date, 7, 2)
        ) AS run_date
    FROM sni_stripe.subscriptions AS s
    INNER JOIN sni_stripe.customers AS c
        ON s.customer = c.id
    INNER JOIN sni_fulcrum.org_stats AS os
        ON c.metadata_organization_id = os.resource_id
    INNER JOIN sni_fulcrum.organizations AS o
        ON os.resource_id = o.resource_id
